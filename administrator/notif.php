	...
	<div id="container">
		<div id="bgwrap">
			...
			<center><h1><br />Welcome Administrator</h1></center>
				...
				$endedsum = mysql_query("SELECT * FROM products WHERE status = 1") or die(mysqli_error());
						$counters = 0;
					...
					$endcheck = mysqli_query($conn,"SELECT * FROM products WHERE status = 1") or die(mysqli_error());
						...
						if ($numberOfRows == 0)
							...
							while($row2 = mysqli_fetch_array($query2)){
								...
								$rows = mysqil_query($conn,"SELECT * needtopay WHERE memberid = '$hewins' AND productid = '$prodid'");
								$numrows1 = MSQLI_NUM_ROWS($rows);
								if ($numrows1 == 0){
